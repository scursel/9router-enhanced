...
describe("xiaomi-mimo executor", () => {
  ...
  it("falls back to cloud transport with API-key auth without mutating desktop credentials", async () => {
    vi.spyOn(mimoAccount, "getMimoAccountCookie").mockResolvedValue(null);
    const execute = vi.spyOn(DefaultExecutor.prototype, "execute").mockImplementation(function ({ model, stream, credentials }) {
      return {
        response: { status: 200 },
        request: {
          url: this.buildUrl(model, stream, 0, credentials),
          headers: this.buildHeaders(credentials, stream, undefined, model),
          credentials,
        },
      };
    });
    const credentials = {
      apiKey: "sk-fake-mimo-key",
      runtimeTransport: OPENAI_T.runtimeTransport,
      providerSpecificData: { region: "ams", mimoPassToken: "fake-pass-token", mimoUserId: "fake-user", custom: "keep" },
    };
    const original = structuredClone(credentials);

    const result = await ex.execute({ model: "mimo-v2.6-flash", body: {}, stream: true, credentials });

    expect(result.request).toMatchObject({
      url: "https://api.xiaomimimo.com/v1/chat/completions",
      headers: { Authorization: "Bearer sk-fake-mimo-key" },
    });
    expect(result.request.credentials.providerSpecificData).toMatchObject({ region: "ams", custom: "keep" });
    expect(result.request.credentials.providerSpecificData).not.toHaveProperty("mimoPassToken");
    expect(result.request.credentials).not.toHaveProperty(COOKIE_KEY);
    expect(credentials).toEqual(original);
    expect(execute).toHaveBeenCalledOnce();
  });

  it("uses a cloud access token when account cookie acquisition fails", async () => {
    vi.spyOn(mimoAccount, "getMimoAccountCookie").mockResolvedValue(null);
    vi.spyOn(DefaultExecutor.prototype, "execute").mockImplementation(function ({ model, stream, credentials }) {
      return {
        response: { status: 200 },
        request: {
          url: this.buildUrl(model, stream, 0, credentials),
    ...
    const credentials = {
      accessToken: "fake-cloud-token",
      runtimeTransport: OPENAI_T.runtimeTransport,
      providerSpecificData: { mimoPassToken: "fake-pass-token" },
    };

    const result = await ex.execute({ model: "mimo-v2.6-pro", body: {}, stream: true, credentials });

    expect(result.request.url).toBe("https://api.xiaomimimo.com/v1/chat/completions");
    expect(result.request.headers.Authorization).toBe("Bearer fake-cloud-token");
  });

  it("rejects account-cookie fallback without cloud credentials before default execution", async () => {
    vi.spyOn(mimoAccount, "getMimoAccountCookie").mockResolvedValue(null);
    const execute = vi.spyOn(DefaultExecutor.prototype, "execute");
    const credentials = {
      runtimeTransport: OPENAI_T.runtimeTransport,
      providerSpecificData: { mimoPassToken: "fake-pass-token" },
    };

    await expect(ex.execute({ model: "mimo-v2.6-flash", body: {}, stream: true, credentials }))
      .rejects.toThrow(/cloud.*credential|API key/i);
    expect(execute).not.toHaveBeenCalled();
    expect(credentials.providerSpecificData.mimoPassToken).toBe("fake-pass-token");
  });

  it("does not mutate credentials when the account cookie route succeeds", async () => {
    vi.spyOn(mimoAccount, "getMimoAccountCookie").mockResolvedValue("serviceToken=fake-cookie");
    const execute = vi.spyOn(DefaultExecutor.prototype, "execute").mockImplementation(function ({ credentials }) {
      return { response: { status: 200 }, credentials };
    });
    const credentials = { providerSpecificData: { region: "ams", mimoPassToken: "fake-pass-token" } };
    const original = structuredClone(credentials);

    const result = await ex.execute({ model: "mimo-v2.6-pro", body: {}, stream: true, credentials });

    expect(result.credentials).not.toBe(credentials);
    expect(result.credentials[COOKIE_KEY]).toBe("serviceToken=fake-cookie");
    expect(credentials).toEqual(original);
    expect(execute).toHaveBeenCalledOnce();
  });
