...
describe("getUsageForProvider(commandcode)", () => {
  ...
  // The fork keeps monthly / purchased / free credits as separate rows (the plan
  // grant is only the monthly one); orgId scoping comes from the official collector.
  it("maps monthly credits vs plan grant, extra balances, and rate windows", async () => {
    mockHappyPath();
    const usage = await getUsageForProvider({
      provider: "commandcode",
      apiKey: "user_test",
    ...
    const monthlyKey = Object.keys(usage.quotas).find((k) => k.startsWith("Monthly credits (USD)"));
    // GOAT grant = 70; monthlyCredits is what remains → used = 57.5
    expect(usage.quotas[monthlyKey]).toMatchObject({ used: 57.5, total: 70, unlimited: false });
    expect(new Date(usage.quotas[monthlyKey].resetAt).toISOString()).toBe("2026-10-01T00:00:00.000Z");
    expect(usage.quotas["Purchased credits (USD)"]).toMatchObject({ total: 1 });
    expect(usage.quotas["Free credits (USD)"]).toMatchObject({ total: 0.5 });
    expect(usage.quotas["5 hour window (USD)"]).toMatchObject({ used: 2, total: 10, unlimited: false });
    expect(usage.quotas["7 day window (USD)"]).toMatchObject({ used: 20, total: 70 });
  });
