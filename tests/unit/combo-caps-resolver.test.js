...
describe("aggregateComboCapabilities: resolveCaps override", () => {
  ...
  it("still applies the conservative rule across members", () => {
    ...
    // Fork rule (combo-capabilities.test.js "maxOutput is the minimum across all
    // models"): a combo can only promise what its weakest member returns, so the
    // fork keeps min where upstream v0.5.91 reports max.
    expect(caps.maxOutput).toBe(131_072);
  });
