...
const ALLOWED_SSO_HOSTS = new Set([ACCOUNT_HOST, ...Object.values(API_BASE_BY_REGION).map((base) => new URL(base).hostname)]);

function validateSsoUrl(rawUrl, baseUrl = `https://${ACCOUNT_HOST}`) {
  let url;
  try { url = new URL(rawUrl, baseUrl); } catch { return null; }
  const trimmed = rawUrl.trim();
  if (trimmed !== rawUrl) return null;
  const authority = /^(?:https:)?\/\/([^/?#]*)/i.exec(trimmed)?.[1];
  if (trimmed.includes("\\") || url.protocol !== "https:" || !ALLOWED_SSO_HOSTS.has(url.hostname)
      || url.username || url.password || url.port
      || (authority && authority.toLowerCase() !== url.hostname)
      || (authority && /:\d+$/.test(authority))) return null;
  return url;
}
...
async function acquireViaDesktopPhases(passJar, proxyOptions, apiBase, sid) {
  ...
  if (!j || typeof j.code !== "number" || j.code !== 0 || !j.location || !j.nonce || !j.ssecurity) {
    failLog(
      ...
      + ` hasNonce=${Boolean(rawNonce || j?.nonce)} hasSsecurity=${Boolean(j?.ssecurity)} responseBytes=${Buffer.byteLength(raw)}`,
    );
    return null;
  }
  ...
  const signedLocation = validateSsoUrl(j.location);
  if (!signedLocation || signedLocation.hostname !== ACCOUNT_HOST) {
    failLog("phase1 rejected unsafe SSO location");
    return null;
  }
  const signedUrl = new URL(signedLocation.toString());
  signedUrl.searchParams.append("clientSign", signatureClientSign(rawNonce || j.nonce, j.ssecurity));
  let current = signedUrl.toString();
  ...
  for (let hop = 0; hop < 8; hop++) {
    ...
    if (res.status >= 300 && res.status < 400 && loc) {
      const next = validateSsoUrl(loc, current);
      if (!next) {
        failLog("phase2 rejected unsafe redirect target");
        return null;
      }
...
async function getServiceCookie(providerSpecificData, proxyOptions) {
  ...
  const promise = (async () => {
    try {
      ...
      console.log(`[mimoAccount] acquire failed (${e?.name || "Error"})`);
...
export async function getMimoAccountCookie(providerSpecificData = null, proxyOptions = null) {
  try {
    ...
    console.log(`[mimoAccount] getMimoAccountCookie failed (${e?.name || "Error"})`);
    return null;
  }
}
