...
import { ANTHROPIC_API_VERSION, OPENAI_COMPAT_BASE, ANTHROPIC_COMPAT_BASE, selectAnthropicBeta, mergeAnthropicBeta, forwardableClientBeta } from "../providers/shared.js";
  ...
  buildHeaders(credentials, stream = true, url, model, body = null) {
    ...
    if (model && (this.provider === "claude"
      ...
      headers["Anthropic-Beta"] = mergeAnthropicBeta(
        selectAnthropicBeta(model, body, clientBeta),
        forwardableClientBeta(model, clientBeta),
      );
    } else if (this.provider === "anthropic" && clientBeta) {
      headers["Anthropic-Beta"] = mergeAnthropicBeta(headers["Anthropic-Beta"], forwardableClientBeta(model, clientBeta));
    }
