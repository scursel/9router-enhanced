...
function cloudRouteCredentials(credentials) {
  const cloudCredentials = { ...credentials };
  delete cloudCredentials[COOKIE_KEY];
  if (credentials?.providerSpecificData) {
    cloudCredentials.providerSpecificData = { ...credentials.providerSpecificData };
    delete cloudCredentials.providerSpecificData.mimoPassToken;
  }
...
export class XiaomiMimoExecutor extends DefaultExecutor {
  ...
  async execute(args) {
    ...
    if (!cookie) {
      const cloudCredentials = cloudRouteCredentials(credentials);
      if (!cloudCredentials.apiKey && !cloudCredentials.accessToken) {
        throw new Error("Xiaomi MiMo cloud fallback requires an API key or access token");
      }
      return super.execute({ ...args, credentials: cloudCredentials });
    }

    const accountCredentials = { ...credentials, [COOKIE_KEY]: cookie };
    const result = await super.execute({ ...args, credentials: accountCredentials });
