...
export default async function proxy(request) {
  ...
  if (!hasSessionCookie && /^\/(fe\/|pass)/.test(pathname) && !pathname.startsWith("/_next")) {
    ...
    console.log(`${new Date().toISOString().slice(11,23)} [mimo-login] no-session ${pathname} (browser cookies present: ${Boolean(cookies)})`);
  }
  if (hasSessionCookie) {
    ...
    if (sess) {
      ...
      try {
        if (isMimoTakeoverPath(pathname)) {
          ...
          return attachSessionCookie(await runTakeover(sess, upstreamUrl, origin), sess, origin);
        }
        if (isAccountProxyPath(pathname)) {
          return attachSessionCookie(await proxyAccountRequest(sess, request, origin), sess, origin);
        }
      } catch (e) {
