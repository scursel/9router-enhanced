...
import { sessionFromRequest, readSessionIdentity, attachSessionCookie, clearedSessionCookie } from "@/lib/mimoLoginSession";
...
export async function GET(request) {
  ...
  if (sess.status !== "done") {
    ...
    return attachSessionCookie(NextResponse.json({ status: "pending", region: sess.region }), sess, request);
  }
  ...
  res.headers.append("Set-Cookie", clearedSessionCookie(request));
  return res;
}
