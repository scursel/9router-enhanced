...
function ValueCells({ item, viewMode, isSummary = false }) {
  if (viewMode === "tokens") {
    return (
      <>
        <td className="px-3 py-3 sm:px-6 text-right text-text-muted">
        ...
        <td className="px-3 py-3 sm:px-6 text-right font-medium">
  ...
  return (
    <>
      ...
      <td className="px-3 py-3 sm:px-6 text-right font-medium text-warning">
          ...
          <thead className="bg-bg-subtle/30 text-text-muted uppercase text-xs">
            <tr>
              {columns.map((col) => (
                <th
                  key={col.field}
                  className={`px-3 py-3 sm:px-6 cursor-pointer hover:bg-bg-subtle/50 ${col.align === "right" ? "text-right" : ""}`}
              ...
              {valueColumns.map((col) => (
                <th
                  key={col.field}
                  className="px-3 py-3 sm:px-6 text-right cursor-pointer hover:bg-bg-subtle/50"
                  onClick={() => onToggleSort(tableType, col.field)}
                >
                  {col.label}{" "}
                  <SortIcon field={col.field} currentSort={sortBy} currentOrder={sortOrder} />
          ...
          <tbody className="divide-y divide-border">
            {groupedData.map((group) => (
              <Fragment key={group.groupKey}>
                ...
                >
                  {/* Phones: a narrow first column so the next columns show and the table reads as scrollable */}
                  <td className="min-w-[9rem] max-w-[11rem] px-3 py-3 sm:max-w-none sm:px-6">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`min-w-0 break-all font-medium transition-colors sm:break-normal ${group.summary.pending > 0 ? "text-primary" : ""}`}>
                        {group.groupKey}
                      </span>
                    </div>
                  </td>
