...
function ComboCard({ combo, getCaps, comboByName = {}, resolveMember, activeProviders = [], copied, onCopy, onEdit, onDelete, strategy = {}, onSetStrategy, comboStats = null, selected = false, onToggleSelect }) {
  ...
  const comboCaps = aggregateComboCapabilities(combo.models, comboByName, { resolveMember, resolveCaps: getCaps });
              ...
              {combo.models.length === 0 ? (
                ...
                combo.models.slice(0, 3).map((model, index) => (
                  <code key={index} className="inline-flex items-center gap-1 rounded bg-black/5 px-1.5 py-0.5 font-mono text-xs text-text-muted dark:bg-white/5">
                    ...
                    <CapacityBadges caps={
                      comboByName[model]
                        ? aggregateComboCapabilities(comboByName[model], comboByName, { resolveMember, resolveCaps: getCaps })
                        : getCaps?.(model)
                    } />
                  </code>
                ))
