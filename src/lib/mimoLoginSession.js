 ...
 *
 * Runs in the server-side proxy and API-route runtimes; uses the existing server JWT secret for signed cookies.
 */

import { createHmac, timingSafeEqual } from "node:crypto";
import { getDashboardSessionSigningSecret, shouldUseSecureCookie } from "@/lib/auth/dashboardSession";

const ACCOUNT_HOST = "account.xiaomi.com";
export const SESSION_COOKIE = "9r_mimo_login";
const SESSION_TTL_MS = 15 * 60 * 1000;
const SESSION_ISSUER = "9router";
const SESSION_AUDIENCE = "mimo-login-session";
const SESSION_PURPOSE = "mimo-login-session";
const SESSION_SECRET = getDashboardSessionSigningSecret();
const API_UA =
...
export function originOf(request) {
  const rawForwardedProto = request.headers?.get?.("x-forwarded-proto")?.split(",")[0]?.trim().toLowerCase();
  const forwardedProto = rawForwardedProto === "https" || rawForwardedProto === "http" ? rawForwardedProto : null;
  const proto = forwardedProto ? `${forwardedProto}:` : (request.nextUrl?.protocol || "http:");
  const host = request.headers?.get?.("host") || request.nextUrl?.host;
  return host ? `${proto}//${host}` : (request.nextUrl?.origin || "http://localhost:20131");
}

// The session (incl. the accumulated cookie jar) travels in the signed SESSION_COOKIE
...
function shouldSecureSessionCookie(requestOrOrigin) {
  if (typeof requestOrOrigin === "string") {
    try {
      return new URL(requestOrOrigin).protocol === "https:" || process.env.AUTH_COOKIE_SECURE === "true";
    } catch {
      return process.env.AUTH_COOKIE_SECURE === "true";
    }
  }
  return shouldUseSecureCookie(requestOrOrigin);
}

export function encodeSessionCookie(sess) {
  const entries = [...sess.jar.values()].map((c) => [c.name, c.value, c.domain, c.path, c.hostOnly !== false]);
  const nowSeconds = Math.floor(sess.createdAt / 1000);
  const base = {
    s: sess.state,
    r: sess.region,
    t: sess.createdAt,
    e: sess.createdAt + SESSION_TTL_MS,
    p: sess.proxyUrl || "",
    j: entries,
    iss: SESSION_ISSUER,
    ...
    iat: nowSeconds,
    exp: Math.floor((sess.createdAt + SESSION_TTL_MS) / 1000),
  };
  ...
  const signingInput = `v3.${encoded}`;
  const signature = createHmac("sha256", SESSION_SECRET).update(signingInput).digest("base64url");
...
export function decodeSessionCookie(value) {
  ...
  const expected = createHmac("sha256", SESSION_SECRET).update(`v3.${parts[1]}`).digest();
  let actual;
  try {
  ...
  const now = Date.now();
  if (!payload || payload.iss !== SESSION_ISSUER || payload.aud !== SESSION_AUDIENCE
      || payload.purpose !== SESSION_PURPOSE || typeof payload.s !== "string" || !payload.s
      || typeof payload.t !== "number" || !Number.isFinite(payload.t)
      || typeof payload.e !== "number" || payload.e !== payload.t + SESSION_TTL_MS
      || typeof payload.iat !== "number" || payload.iat !== Math.floor(payload.t / 1000)
      || typeof payload.exp !== "number" || payload.exp !== Math.floor(payload.e / 1000)
      || payload.t > now || now >= payload.e || now - payload.t > SESSION_TTL_MS
      || !(payload.r in MIMO_BASES) || !Array.isArray(payload.j)
      || (payload.p !== "" && typeof payload.p !== "string")) return null;
  const jar = new Map();
  for (const raw of payload.j) {
    if (!Array.isArray(raw) || (raw.length !== 4 && raw.length !== 5)
        || raw.slice(0, 4).some((item) => typeof item !== "string")
        || (raw.length === 5 && typeof raw[4] !== "boolean")) return null;
    const [name, val, domain, path, hostOnly = false] = raw;
    if (!name || !domain || !path.startsWith("/")) return null;
    jar.set(`${name}|${domain}|${path}`, { name, value: val, domain, path, hostOnly });
  }
  return {
    state: payload.s,
    region: payload.r,
    proxyUrl: payload.p || null,
    jar,
    status: "pending",
    createdAt: payload.t,
...
export function sessionFromRequest(request) {
  ...
  const decoded = decodeSessionCookie(raw || null);
  if (decoded && typeof decoded.then === "function") return null;
  return decoded;
}
...
export function attachSessionCookie(response, sess, request) {
  const headers = new Headers(response.headers);
  const secure = shouldSecureSessionCookie(request);
  headers.append(
    "Set-Cookie",
    `${SESSION_COOKIE}=${encodeSessionCookie(sess)}; Path=/; HttpOnly; SameSite=Lax${secure ? "; Secure" : ""}; Max-Age=${Math.floor(SESSION_TTL_MS / 1000)}`,
  );
...
export function clearedSessionCookie(request) {
  return `${SESSION_COOKIE}=; Path=/; HttpOnly; SameSite=Lax${shouldSecureSessionCookie(request) ? "; Secure" : ""}; Max-Age=0`;
}
...
function domainMatch(cookieDomain, host) {
  ...
  if (!d || !h || d.indexOf(".") < 0) return false;
...
function parseSetCookie(raw, requestUrl) {
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    if (k === "domain" && v) {
      const candidate = v.replace(/^\./, "").toLowerCase();
      const responseHost = url.hostname.toLowerCase();
      if (!candidate.includes(".") || !domainMatch(candidate, responseHost)) return null;
...
function cookieHeaderFor(sess, targetUrl) {
  ...
  for (const c of sess.jar.values()) {
    const cookieHost = String(c.domain || "").replace(/^\./, "").toLowerCase();
    if (c.hostOnly ? u.hostname.toLowerCase() !== cookieHost : !domainMatch(c.domain, u.hostname)) continue;
...
export async function ensureServiceSession(sess) {
  ...
  try {
    ...
    log(`exchanging account service session (region=${sess.region}, egress=${sess.proxyUrl ? "proxy" : "direct"}) ...`);
    ...
    log(`exchange error (${e?.name || "Error"})`);
    _exchangeBackoff.set(bkKey, Date.now());
    return false;
  }
}
...
// egress IP of THESE requests. Configured or discovered egress applies only
// to non-CN regions and rides the signed cookie as sess.proxyUrl. CN sessions
// never proxy (null -> direct).
...
async function socksFetch(url, init, proxyUrl) {
  if (!_socksPromise) {
    _socksPromise = import("socks-proxy-agent")
      ...
      .catch((e) => {
        console.log(`${new Date().toISOString().slice(11,23)} [mimo-login] socks-proxy-agent unavailable (${e?.name || "Error"})`);
        return null;
      });
  }
...
async function loginFetch(url, init, sessionProxyUrl = null) {
  ...
  const proxyOptions = { enabled: true, url: sessionProxyUrl, strictProxy: true };
  if (/^socks/i.test(sessionProxyUrl)) return socksFetch(url, init, sessionProxyUrl);
  if (!_pafPromise) {
    _pafPromise = import("../../open-sse/utils/proxyFetch.js")
      .then((m) => m.proxyAwareFetch)
      .catch((error) => {
        console.log(`${new Date().toISOString().slice(11,23)} [mimo-login] proxyAwareFetch unavailable (${error?.name || "Error"})`);
        return null;
      });
  }
  const paf = await _pafPromise;
  if (!paf) throw new Error("configured MiMo login proxy is unavailable");
  return paf(url, init, proxyOptions);
}
...
export async function proxyAccountRequest(sess, req, origin) {
  ...
  if (init.method !== "GET" && init.method !== "HEAD") {
    ...
    if (buf.length && /urlencoded|json|text/i.test(ctBody)) {
      ...
      if (after !== before) {
        ...
        console.log(`${new Date().toISOString().slice(11, 23)} [mimo-login] body rewrite anomaly: local callback marker remains (bytes=${buf.length})`);
      }
    }
    init.body = buf;
  }
  ...
  for (let hop = 0; hop < 8; hop++) {
    const requestCookies = cookieHeaderFor(sess, current);
    ...
    if (res.status >= 300 && res.status < 400 && loc) {
      let nextAbs;
      try { nextAbs = new URL(loc, current); } catch {
        return new Response("unsafe redirect URL", { status: 502, headers: { "Cache-Control": "no-store" } });
      }
      if (nextAbs.hostname === ACCOUNT_HOST) {
        const rawLocation = loc.trim();
        const authority = /^(?:https:)?\/\/([^/?#]*)/i.exec(rawLocation)?.[1]?.toLowerCase();
        const isRelative = !/^[a-z][a-z0-9+.-]*:/i.test(rawLocation) && !rawLocation.startsWith("//");
        const isProtocolRelative = rawLocation.startsWith("//");
        if (rawLocation.includes("\\") || nextAbs.protocol !== "https:" || nextAbs.username || nextAbs.password
            || (!isRelative && authority !== ACCOUNT_HOST)
            || (isProtocolRelative && (nextAbs.port || authority !== ACCOUNT_HOST))) {
          return new Response("unsafe account redirect", { status: 502, headers: { "Cache-Control": "no-store" } });
        }
      ...
      const mimoBase = Object.values(MIMO_BASES).find((base) => new URL(base).hostname === nextAbs.hostname);
      if (mimoBase) {
        const rawLocation = loc.trim();
        const authority = /^(?:https:)?\/\/([^/?#]*)/i.exec(rawLocation)?.[1]?.toLowerCase();
        if (rawLocation.includes("\\") || nextAbs.protocol !== "https:" || nextAbs.port || nextAbs.username || nextAbs.password
            || (authority && (authority !== nextAbs.hostname || /:\d+$/.test(authority)))) {
          return new Response("unsafe MiMo redirect", { status: 502, headers: { "Cache-Control": "no-store" } });
        }
      ...
      return new Response("unapproved redirect host", { status: 502, headers: { "Cache-Control": "no-store" } });
    }
    break;
  }
  ...
  return res.text().then((rawBody) => {
    ...
    if (/^\/(pass|sts)/.test(reqPath) || res.status >= 400) {
      ...
      if ((m && m[1] !== "0") || res.status >= 400) {
        console.log(`${new Date().toISOString().slice(11,23)} [mimo-login] upstream response rejected http=${res.status} code=${m ? m[1] : "?"} route=${/^\/(pass|sts)/.exec(reqPath)?.[1] || "other"} bodyBytes=${Buffer.byteLength(body)}`);
      }
    }
...
export async function runTakeover(sess, upstreamUrl, origin) {
  ...
  const targetHost = (() => { try { return new URL(upstreamUrl).hostname; } catch { return "invalid"; } })();
  log(`sts-nav host=${targetHost} origin=${origin} | ${idSnap()}`);
  const id = readSessionIdentity(sess);
...
export const __test__ = { STRIP_UPSTREAM_HEADERS, buildBrowserResponse, cookieHeaderFor };
