...
import { CATALOG_FILE, CATALOG_RAW_FILE, CATALOG_VERSION, invalidateCatalog, installCatalogSource, normalizeLifecycleStatus } from "open-sse/providers/catalogOverride.js";
...
export async function syncModelCatalog() {
  ...
  try {
    ...
    if (response.status === 304) {
      ...
      const serialized = JSON.stringify({ v: CATALOG_VERSION, etag, syncedAt: Date.now(), models, providers, costs, lifecycle });
